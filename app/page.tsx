...
export default function SenderPage() {
    ...
    const handlePaymentAndSend = async () => {
        ...
        try {
            // Updated to ensure it hits the correct local API path
        ...
        } catch (err) { alert("Check if /api/checkout/route.ts exists in your GitHub."); }
    };
    ...
    return (
        <main style={{ height: '100vh', width: '100vw', background: '#000', position: 'relative', overflow: 'hidden', fontFamily: 'sans-serif' }}>
                ...
                <div style={{ position: 'relative', zIndex: 10, height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        ...
                        <div style={{ position: 'relative', width: '450px', minHeight: '400px', background: 'rgba(0,0,0,0.5)', borderRadius: '25px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', border: '1px solid #0070f3', perspective: '1000px', marginBottom: '20px' }}>
                            <div style={{ position: 'absolute', top: '-20px', width: '80%', background: '#0070f3', color: '#fff', padding: '10px 0', borderRadius: '50px', textAlign: 'center', fontWeight: 'bold', zIndex: 50 }}>SEND A HEART IN A BOX</div>
                            
                            <div style={{ position: 'relative', width: '100%', display: 'flex', justifyContent: 'center' }}>
                                <img src="https://storage.googleapis.com/simple-bucket-27/blue-box.png" style={{ width: '90%', zIndex: 5 }} alt="Box" />
                                {selectedTiles.length > 0 && (
                                    <div style={{ position: 'absolute', bottom: '85px', left: 0, right: 0, display: 'flex', flexDirection: 'row', justifyContent: 'center', gap: '12px', zIndex: 30 }}>
                                        {selectedTiles.map((tile, idx) => (
                                            <div key={idx} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                                ...
                                                {/* Blue Text labels with high z-index to stay on top */}
                                                <span style={{ color: '#0070f3', fontSize: '1rem', fontWeight: 'bold', background: 'rgba(0,0,0,0.8)', padding: '2px 10px', borderRadius: '10px', marginTop: '8px', zIndex: 100 }}>{tile}</span>
                                            </div>
                                        ))}
                                    </div>
                                )}
                        ...
                        <div style={{ width: '550px', background: '#0070f3', color: '#fff', padding: '12px 20px', borderRadius: '50px', fontWeight: 'bold', fontSize: '0.85rem', textAlign: 'center', marginBottom: '20px' }}>
                            After you're done writing please click on the most important words to transform them into "vibes" (optional).
                        </div>

                        <div style={{ width: '650px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
                    ...
                    <div style={{ position: 'absolute', right: '50px', top: '15%', display: 'flex', flexDirection: 'column', gap: '20px', alignItems: 'center' }}>
                        ...
                        <div style={{ display: 'flex', gap: '15px' }}>
                            <button onClick={() => setIsCinematicView(true)} style={{ background: 'rgba(0,0,0,0.8)', border: '2px solid #0070f3', borderRadius: '30px', padding: '15px 25px', cursor: 'pointer', fontSize: '2rem', color: '#fff' }}>👁️</button>
            ...
            {isCinematicView && (
                <button onClick={() => setIsCinematicView(false)} style={{ position: 'absolute', top: '20px', right: '20px', zIndex: 100, background: 'rgba(0,0,0,0.8)', border: '2px solid #fff', borderRadius: '30px', padding: '10px 25px', color: '#fff', fontWeight: 'bold', cursor: 'pointer' }}>EDIT ✏️</button>
            )}
        </main>
    );
}
